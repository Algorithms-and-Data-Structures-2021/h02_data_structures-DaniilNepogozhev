...
ArrayList::ArrayList(int capacity) : capacity_{capacity} {
  if (capacity <= 0) {
    ...
    data_ = new Element[kInitCapacity]{};
...
void ArrayList::Clear() {
    for (int pos = 0; pos<capacity_; pos++ ) {
        data_[pos] = Element::UNINITIALIZED;
    }
    size_ = 0;
}
...
void ArrayList::resize(int new_capacity) {
  ...
  std::fill(new_data + capacity_, new_data + new_capacity, Element::UNINITIALIZED);
